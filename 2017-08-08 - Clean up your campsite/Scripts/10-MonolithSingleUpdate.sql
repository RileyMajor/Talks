...
DROP PROCEDURE IF EXISTS MonolithProcessSingleUpdate;
GO

CREATE PROCEDURE MonolithProcessSingleUpdate
	...
	BEGIN
		...
		*/
		...
		BEGIN
			...
			SELECT
				@OrderInvoiceAmount = @OrderGrossCharges,
			...
			BEGIN
				SELECT
					@OrderNetCharges = @OrderGrossCharges * (1 - @CustomerVolumeDiscount),
					@OrderInvoiceDate = @Now;
			...
			SET @OrderStatusIDNew = @OrderStatusInvoiced;
		...
		BEGIN
			...
			BEGIN
				...
				SET @OrderStatusIDNew = @OrderStatusRejected;
			...
			SET @OrderStatusIDNew = @OrderStatusProcessed;
