...
DROP PROCEDURE IF EXISTS ProcessOrdersNoCursor;
GO

CREATE PROCEDURE ProcessOrdersNoCursor
...
BEGIN
	...
	UPDATE		o
	SET			o.OrderStatusID = isNull(op.OrderStatusIDNew,o.OrderStatusID),
				o.OrderInvoiceAmount = isNull(op.OrderInvoiceAmount,o.OrderInvoiceAmount),
				o.OrderInvoiceDate = isNull(op.OrderInvoiceDate,o.OrderInvoiceDate)
	OUTPUT		op.EmailTo,
				op.EmailSubject,
				op.EmailBody
	INTO		@Emails
	...
	DECLARE EmailList CURSOR LOCAL STATIC FORWARD_ONLY FOR
		SELECT
			EmailTo,
			EmailSubject,
			EmailBody
	...
	FETCH NEXT FROM EmailList INTO @EmailTo, @EmailSubject, @EmailBody;
